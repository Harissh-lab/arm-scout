...
export function DashboardScreen({ onNavigateToScout, vehicles, selectedVehicleId, onSelectVehicle }: DashboardScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFAF9] dark:bg-slate-950 pb-20 sm:pb-24">
      ...
      {/* Live Map Widget */}
      <div className="px-3 sm:px-4 mt-4 sm:mt-6">
        <h3 className="text-[#484B6A] dark:text-slate-300 text-xs sm:text-sm mb-2 sm:mb-3">Hazard Scout Status</h3>
        <button 
          onClick={onNavigateToScout}
          ...
          <Card className="bg-white dark:bg-slate-900/90 border-gray-200 dark:border-slate-700/50 backdrop-blur-xl overflow-hidden cursor-pointer hover:border-[#001E50] dark:hover:border-[#0070E1]/50 transition-all">
            <CardContent className="p-4">
              <div className="flex items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#0070E1]/10 flex items-center justify-center">
                    <ShieldAlert className="w-5 h-5 text-[#0070E1]" />
                  </div>
                  <div>
                    <p className="text-[#1F2F57] dark:text-slate-200 text-sm">Camera Detection</p>
                    <p className="text-[#484B6A] dark:text-slate-400 text-xs">Tap to view & manage</p>
                  </div>
                </div>
                <Badge className={`${hazardService.getActiveHazards().length > 0 ? 'bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-500/30' : 'bg-green-50 dark:bg-green-500/20 text-green-600 dark:text-green-400 border-green-200 dark:border-green-500/30'} text-xs`}>
                  {hazardService.getActiveHazards().length} Active
                </Badge>
              </div>
              {hazardService.getActiveHazards().length > 0 ? (
                <div className="bg-slate-100 dark:bg-slate-800/50 rounded-lg p-2.5 border border-slate-200 dark:border-slate-700/30">
                  <div className="flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-amber-400" />
                    <p className="text-[#484B6A] dark:text-slate-300 text-xs">
                      {hazardService.getActiveHazards().length} hazard{hazardService.getActiveHazards().length > 1 ? 's' : ''} detected in your area
                    </p>
                  </div>
                </div>
              ) : (
                <div className="bg-green-50 dark:bg-green-500/10 rounded-lg p-2.5 border border-green-200 dark:border-green-500/20">
                  <div className="flex items-center gap-2">
