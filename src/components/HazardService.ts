// Hazard Management Service with Camera Detection & Coordinate-based Alerts
// Integrates with Raspberry Pi camera detection and GPS coordinates

import { cameraDetectionService, CameraDetection, GPSPosition } from './CameraDetectionService';
import { coordinateDatabase, CoordinateHazard, ProximityAlert } from './CoordinateDatabase';
...
export class HazardManagementService {
  ...
  private cameraDetectionUnsubscribe: (() => void) | null = null;
  private proximityUnsubscribe: (() => void) | null = null;
  ...
  // ===== CAMERA & COORDINATE INTEGRATION =====

  // Setup camera detection integration
  private setupCameraIntegration(): void {
    this.cameraDetectionUnsubscribe = cameraDetectionService.onDetection((detection) => {
      this.handleCameraDetection(detection);
    });
  }

  // Setup proximity monitoring
  private setupProximityMonitoring(): void {
    this.proximityUnsubscribe = coordinateDatabase.onProximityAlert((alerts) => {
      this.proximityAlerts = alerts;
      // Trigger any UI updates or notifications
      console.log('Proximity alerts:', alerts);
    });

    // Start monitoring with GPS position provider
    coordinateDatabase.startProximityMonitoring(() => {
      const pos = cameraDetectionService.getCurrentPosition();
      if (!pos) return null;
      return {
  ...
  // Handle camera detection from Raspberry Pi
  private handleCameraDetection(detection: CameraDetection): void {
    // Add to coordinate database
    const hazardId = coordinateDatabase.addHazard({
      type: detection.type,
      coordinates: {
      ...
      severity: this.determineSeverity(detection.type, detection.confidence),
    });

    // Also add to legacy hazard system
    const locationName = `${detection.coordinates.latitude.toFixed(4)}, ${detection.coordinates.longitude.toFixed(4)}`;
    this.addHazard({
      type: detection.type,
      severity: this.determineSeverity(detection.type, detection.confidence),
      location: {
    ...
    console.log('Camera detected hazard:', detection.type, 'at', locationName);
  }

  // Determine severity based on hazard type and confidence
  private determineSeverity(type: string, confidence: number): 'high' | 'medium' | 'low' {
    if (type === 'accident' || type === 'flood' || type === 'roadblock') {
      return 'high';
    }
    if (type === 'debris' || type === 'pothole') {
      return confidence > 85 ? 'high' : 'medium';
    }
    return 'medium';
  }

  // Start camera and GPS monitoring
  async startMonitoring(): Promise<boolean> {
    const gpsStarted = await cameraDetectionService.startGPSMonitoring();
    if (!gpsStarted) {
      console.error('Failed to start GPS monitoring');
      return false;
    }
    return true;
  }

  // Stop camera and GPS monitoring
  stopMonitoring(): void {
    cameraDetectionService.stopGPSMonitoring();
    coordinateDatabase.stopProximityMonitoring();
  }
  ...
  getCurrentGPSPosition(): GPSPosition | null {
    return cameraDetectionService.getCurrentPosition();
  }

  // Get coordinate database instance
  getCoordinateDatabase() {
    return coordinateDatabase;
  }

  // Get camera detection service
  getCameraService() {
    return cameraDetectionService;
  }

  // Simulate camera detection (for testing)
  simulateCameraDetection(type: 'debris' | 'pothole' | 'roadblock' | 'accident' | 'flood' | 'construction' = 'debris'): void {
    cameraDetectionService.simulateDetection(type);
  }

  // ===== END CAMERA & COORDINATE INTEGRATION =====
  ...
  destroy(): void {
    ...
    if (this.cameraDetectionUnsubscribe) {
      this.cameraDetectionUnsubscribe();
    }
    if (this.proximityUnsubscribe) {
      this.proximityUnsubscribe();
    }
    this.stopMonitoring();
  }
}
...
// Re-export coordinate database types for easier access
export type { CoordinateHazard, ProximityAlert } from './CoordinateDatabase';
export type { CameraDetection, GPSPosition } from './CameraDetectionService';
