import { ArrowLeft, ShieldAlert, Volume2, Bell, Eye, Shield, Radio, BellRing, Moon, Check, X, Clock, Users, Camera } from "lucide-react";
...
export function HazardScoutSettingsScreen({ onBack }: HazardScoutSettingsScreenProps) {
  ...
  const [piCameraUrl, setPiCameraUrl] = useState<string>(
    cameraDetectionService.getPiCameraUrl()
  );
  const [piApiUrl, setPiApiUrl] = useState<string>(
    cameraDetectionService.getPiApiUrl()
  );
  ...
  useEffect(() => {
    ...
    setPiCameraUrl(cameraDetectionService.getPiCameraUrl());
    setPiApiUrl(cameraDetectionService.getPiApiUrl());
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24">
      ...
      <div className="px-4 mt-6">
        <h3 className="text-slate-300 text-sm mb-3">Raspberry Pi Camera</h3>
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <p className="text-slate-200 text-sm mb-1">Camera Stream URL</p>
                <p className="text-slate-400 text-xs mb-3">
                  Enter your Raspberry Pi camera stream endpoint
                </p>
                <Input
                  type="text"
                  value={piCameraUrl}
                  onChange={(e) => setPiCameraUrl(e.target.value)}
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <p className="text-slate-200 text-sm mb-1">Pi API Endpoint</p>
                <p className="text-slate-400 text-xs mb-3">
                  Detection API endpoint (optional)
                </p>
                <Input
                  type="text"
                  value={piApiUrl}
                  onChange={(e) => setPiApiUrl(e.target.value)}
                  placeholder="http://raspberrypi.local:5000/api"
                  className="bg-slate-800/50 border-slate-700 text-slate-200 text-sm"
                />
              </div>
            </div>
            ...
            <Button
              onClick={() => {
                cameraDetectionService.configurePiCamera(piCameraUrl, piApiUrl);
                toast.success("Pi Camera Configured", {
                  description: "Camera settings saved successfully.",
                  duration: 3000,
                });
              }}
              className="w-full bg-[#0070E1] hover:bg-[#0056b3] text-white"
            >
            ...
            <div className="mt-3 bg-blue-500/10 rounded-lg p-3 border border-blue-500/20">
              <p className="text-blue-300 text-xs leading-relaxed">
                💡 <strong>Setup Guide:</strong> Make sure your Raspberry Pi is on the same network. 
                Default stream URL is usually on port 8080. For API integration, use port 5000.
              </p>
            </div>
          </CardContent>
        </Card>
